...
app.get('/', async (req, res) => {
    try {
        ...
        if(req.query.sortBy) {
            restaurants = await ResModel.find({}).sort({"restaurant_id": req.query.sortBy.toLowerCase()})
        } else {
            restaurants = await ResModel.find({})
        }
...
app.get('/cuisine/:cuisine', async (req, res) => {
    try {
        const restaurants = await ResModel.find({cuisine: req.params.cuisine})
...
app.get('/delicatessen', async (req, res) => {
    try {
        const restaurants = await ResModel.find({cuisine: 'delicatessen', city: {$ne: 'Brooklyn'}})
        res.send(restaurants)
    } catch(err) {
        console.log(err)
        res.status(500).send({error: err.toString()})
...
app.get('/seed', async (req, res) => {
    try {
        await ResModel.deleteMany({})
        await ResModel.insertMany(
            [{
              "address": {
                "building": "1008",
